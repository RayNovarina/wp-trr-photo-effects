...
function exp_scroll_trigger( event, direction, type, tag ) {
  var profile_idx_triggering_event = parseInt( $( globals.bio_containers_class_ref ).find( tag ).attr('profile-idx') ),
  ...
  if ( $( globals.bio_containers_class_ref ).attr('active_profile_idx') == undefined ) {
    // Upon load, init case.
    ...
    scroll_result_msg = ". Moving out of view: '." +
                        $( $('.profile-container').toArray()[ scrolling_profile_idx_out_of_view ] ).attr('id') +
                        "' ";
    ...
    scroll_result_msg = ". Moving into view: '." +
                        $( $('.profile-container').toArray()[ scrolling_profile_idx_into_view ] ).attr('id') +
                        "' ";
  }

  var scrolling_bio_into_view = $( $('.bio-container').toArray()[ scrolling_profile_idx_into_view ] );

  exp_statusLog( "  ..*18: Scrolled (" + direction + ") '" + tag + ': profile-idx ' +
                 $( globals.bio_containers_class_ref ).find( tag ).attr('profile-idx') +
                 scroll_result_msg +
                 // "'.  Active halftone profile: " + active_bio.attr('profile-idx') + ":" +
                 // active_bio.attr('active_id') +
                 "*" );
  if ( (scrolling_profile_idx_into_view + '') == $( globals.bio_containers_class_ref ).attr('active_profile_idx') ) {
