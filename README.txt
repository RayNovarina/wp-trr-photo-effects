...
==========================
  ...
  TrrPhotoEffects = {
    statusLog: function( msg ) {
      ...
      console.log(msg);
    }, // end TrrPhotoEffects.Controller

    Controller: function( parms ) {
      this.config = parms;
      // NOTE: ??? after callback 'this' points to window elem, not our instance. So all
      // references going forward refer to 'self', not 'this'
      this.statusLog( "  ..*1-creating TrrPhotoEffects.Controller with config parms of " +
                     "scroll_starts_animation: '" + parms.scroll_starts_animation +
                     "'.  click_starts_animation: '" + parms.click_starts_animation +
                     "'. *" );
      var that = this;
